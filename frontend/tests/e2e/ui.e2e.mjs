...
import fs from 'fs';
const BASE = process.env.APP_URL || 'http://localhost:9722';
...
await step('admin: setujui struk lain', async () => { await a.goto(BASE + '/admin/struk'); await a.locator('tbody tr.clickable-row').first().click(); await a.getByText('Hasil validasi otomatis').waitFor(); await a.getByRole('button', { name: 'Setujui' }).click(); await a.getByRole('button', { name: 'Setujui', exact: true }).last().click(); await a.getByText('Keputusan (final)').waitFor({ timeout: 6000 }); });
...
await step('profil: preferensi notifikasi (email/WA belum tersedia → disabled)', async () => { await login(m2, 'demo@alpaka.local', 'Demo-Alpaka-2026'); await m2.getByText('Poin tersedia').waitFor(); await m2.goto(BASE + '/profil'); await m2.getByText('Preferensi notifikasi').waitFor(); await m2.getByText('belum tersedia').first().waitFor(); if (!(await m2.getByRole('switch', { name: 'Notifikasi email' }).isDisabled())) throw new Error('switch email harus disabled'); await shot(m2, 'm-profil'); });
...
await step('admin: pengaturan program memuat teks syarat', async () => { await a2.goto(BASE + '/admin/pengaturan'); await a2.getByText('Syarat program & kebijakan privasi').waitFor(); const v = await a2.locator('textarea').first().inputValue(); if (!v.includes('SYARAT PROGRAM')) throw new Error('terms kosong'); });
...
await step('mobile: riwayat redeem tanpa tabel horizontal-scroll', async () => { await mob2.goto(BASE + '/redeem'); await mob2.getByRole('tab', { name: 'Riwayat Redeem' }).click(); await mob2.getByText('Menunggu Persetujuan').first().waitFor(); await noOverflow(mob2, 'redeem'); await shot(mob2, 'mob-redeem2'); });
